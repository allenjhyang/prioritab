...
function CheckTimersToggle() {
  if ($("#hide-timers-checkbox").is(":checked")) {
        $("#top-row").hide();
        chrome.storage.sync.set({
            'user-hide-timers': 'true'
        });
    } else {
        $("#top-row").show();
        chrome.storage.sync.set({
            'user-hide-timers': 'false'
        });
    }
}
...
function CountdownDay() {
    ...
    timeoutWrapper.clearTimeout();
    timeoutWrapper.setTimeout(CountdownDay, totalDayMS / 100)
}
...
function CountdownWorkday() {
    ...
    chrome.storage.sync.get(['user-workday-start', 'user-workday-end'], function(retrieved) {
        ...
        if (now < workdayStart) {
            progressPCT = 0;
            timeoutWrapper.clearTimeout();
            timeoutWrapper.setTimeout(CountdownWorkday, workdayStart - now)
        } else if (now > workdayEnd) {
            progressPCT = 100;
            timeoutWrapper.clearTimeout();
            timeoutWrapper.setTimeout(CountdownWorkday,  24 * 60 * 60 * 1000 - (now - workdayStart))
        } else {
            var progressMS = now - workdayStart,
                totalDayMS = workdayEnd - workdayStart;
            progressPCT = progressMS / totalDayMS * 100;
            timeoutWrapper.clearTimeout();
            timeoutWrapper.setTimeout(CountdownWorkday, totalDayMS / 100)
        }
...
window.onload = function() {
    SetDateTimeFormat();
      chrome.storage.sync.get('user-hide-timers', function(result) {
        if (result['user-hide-timers'] === "true") {
            $("#top-row").hide();
            $("#hide-timers-checkbox").prop("checked", !0)
        }
    });
    ...
    $("#workday-time-save").click(function(e) {
      ...
      $('#hide-timers-checkbox').click(function(e) {
        CheckTimersToggle()
    });
