...
$(function () {
  ...
  chrome.storage.local.get(listCounters, function (result) {
  ...
  // Load todo list keys
  chrome.storage.local.get('todo-orders', function (retrieved) {
    ...
    chrome.storage.local.get(orderListLeft, function (result) {
    ...
    chrome.storage.local.get(orderListMid, function (result) {
    ...
    chrome.storage.local.get(orderListRight, function (result) {
  ...
  $.subscribe('/add/', function () {
    ...
    if (todoToAdd) {
      ...
      chrome.storage.local.set(counterToSave);

      // Append a new list item with the value of the new todo list
      chrome.storage.local.get(newTodoID, function (result) {
  ...
  $.subscribe('/remove/', function ($this) {
    ...
    chrome.storage.local.remove(parentId);
  ...
  var reassignToList = function (inputDict) {
    ...
    items.each(function () {
      if (this.id.indexOf(target) < 0) {
        ...
        chrome.storage.local.get(oldID, function (retrieved) {
